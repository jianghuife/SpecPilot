...
> **Note:**  
> This guide is mainly for agents and LLMs to follow when maintaining,  
> generating, or refactoring React single-page applications. Humans  
...
7. [Advanced Patterns](#7-advanced-patterns) — **LOW**
   ...
   - 7.4 [Read Latest Callback Values with useEffectEvent](#74-read-latest-callback-values-with-useeffectevent)
...
This matters when `getFlag` hits the network, a feature-flag service, or other expensive async work: skipping it when `someCondition` is false removes that cost on the cold path.
...
> **TypeScript warning:** Some libraries don't ship `.d.ts` files for their deep import paths. Verify the library exports types for its subpaths before using direct imports.
...
Use the `typeof window !== 'undefined'` check only when the module touches browser-only globals.
...
Prefer explicit maps or literal paths so the set of reachable files stays narrow and predictable. This is the same rule whether you are choosing modules with `import()` or resolving asset/content paths in build tooling.

When analysis becomes too broad, the cost is real:

- Larger client chunks
- Slower builds
- Worse cold starts
- More memory use
...
useEffect(() => {
  ...
  setBrowserTitle(`${pageTitle} | My App`);
}, [pathname, pageTitle]);
```
...
Use `useTransition` for non-urgent UI updates that need a pending indicator. It
can replace manual pending state for transition work, but it is not a blanket
replacement for request loading state.
...
function SearchResults() {
  ...
  const handleSearch = (value: string) => {
    ...
    startTransition(async () => {
      const data = await fetchResults(value);
      // React currently requires wrapping state updates after await.
      startTransition(() => {
        setResults(data);
      });
    });
...
**Important:** wrap state updates after an awaited async request in another
`startTransition`; otherwise React will not mark those updates as transition
work. Also handle request ordering when multiple requests can finish out of
order.
...
function isLoggedIn(): boolean {
  ...
  isLoggedInCache = readCookieHeader().includes("auth=");
  return isLoggedInCache;
}
...
`localStorage`, `sessionStorage`, and cookie reads are synchronous and expensive. Cache reads in memory.
...
function getCookie(name: string) {
  if (!cookieCache) {
    cookieCache = Object.fromEntries(readCookieHeader().split("; ").map((c) => c.split("=")));
  }
...
If storage can change externally (another tab or cookie updates), invalidate cache:
...
Effect Events are not stable dependency values. They are designed to be omitted
from effect dependency arrays while still reading the latest committed values.
Keep them local to the effect that uses them.

### 7.4 Read Latest Callback Values with useEffectEvent
...
Effect Events are not stable dependency values. Keep them local to effects and
omit them from dependency arrays.
