...
**Adapter** — a concrete thing that satisfies an interface at a seam. Describes _role_ (what slot it fills), not substance (what's inside). Distinct from **Implementation**: a thing can be a small adapter with a large implementation (a Postgres repo) or a large adapter with a small implementation (an in-memory fake). Reach for "adapter" when the seam is the topic; "implementation" otherwise.
...
- **Depth as ratio of implementation-lines to interface-lines** (a common paraphrase of Ousterhout — his actual definition is functionality relative to interface complexity): rewards padding the implementation. We use depth-as-leverage instead.
