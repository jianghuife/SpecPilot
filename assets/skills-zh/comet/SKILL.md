...
Understand Anything 负责 CODEBASE MAP — 架构、组件、层级、已有实现边界
CodeGraph 负责 SYMBOL/IMPACT INDEX — 符号定位、调用关系、影响面分析
```

Understand Anything 和 CodeGraph 是推荐的上下文增强工具：前者帮助设计阶段理解既有代码结构，后者帮助构建阶段定位符号和分析影响面。它们不能替代 OpenSpec 的需求事实源、Superpowers 的流程门、项目测试、构建或验证结论；不可用时按普通文件阅读和搜索继续。
...
| `/comet-design` | 2. 深度设计 | Superpowers + Understand Anything（推荐） | Design Doc、delta spec |
| `/comet-build` | 3. 计划与构建 | Superpowers + CodeGraph（推荐） | 实施计划、代码提交 |
...
> - **用户决策点模板**：按 `comet/reference/decision-templates.md` 查阅
