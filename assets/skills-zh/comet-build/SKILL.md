...
- 已将关键构建/测试结果记录到 evidence ledger（推荐）：使用 `"$COMET_BASH" "$COMET_RUN" <change-name> build -- <command...>` 执行命令并自动记录 pass/fail；非命令类事件可继续用 `"$COMET_BASH" "$COMET_EVIDENCE" record ...` 手动记录；若证据脚本不可用，不阻塞流程，但需在验证报告中写明缺失
