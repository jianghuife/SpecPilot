...
Run `specpilot session show --json`, then `specpilot status --json`. Use the local session pointer
when valid; status marks it stale when its change or task no longer exists. Otherwise list open
...
Projects may optionally enable a short per-turn state breadcrumb. Treat it only as a pointer to
the repository state; use `session show`, `status`, and `context list` for authoritative detail.
