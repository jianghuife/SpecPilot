...
主入口 `/comet` 支持当前 Spec 状态检测，适用于长任务——中途关闭当前 AI 编码会话后，回来只需 `/comet 继续`，Comet 会自动读取活跃的 Spec（多个则列出选择），动态识别当前执行到哪个阶段，继续往下执行。
...
这个路径适合 `comet init` 暂未内置支持的平台。它可能不会执行 `comet init` 提供的完整初始化，例如为所选平台安装 OpenSpec/Superpowers 或创建项目工作目录。
