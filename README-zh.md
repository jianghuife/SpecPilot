...
> [抖音](https://www.douyin.com/search/SpecPilot?aid=cd8fcc82-498b-4d59-8860-617deb719412&modal_id=7646429015808936293&type=general)

**面向 AI 编程 Agent 的 Spec 驱动工作流工具** — 从创意到归档，一条命令。

OpenSpec 处理 **WHAT**（大纲、提案、spec 生命周期、归档）。

Superpowers 处理 **HOW**（技术设计、规划、执行、收尾）。

SpecPilot 将二者串联为五阶段自动化流水线。
...
**SpecPilot 合并了两者的强项**，将核心流程整合为 5 个阶段

安装后的工作流入口 `/comet` 支持当前 Spec 状态检测，适用于长任务——中途关闭当前 AI 编码会话后，回来只需在 AI 编码 Agent 中调用 `/comet`，SpecPilot 会自动读取活跃的
Spec（多个则列出选择），动态识别当前执行到哪个阶段，继续往下执行。

同时，SpecPilot具备Spec全生命周期管理能力，运行过程中能够将 OpenSpec 的 change/spec 制品与 Superpowers
...
长期使用 Skill 的人都知道，这些能力是可以自由组合的，但具体怎么做依然需要真正的实践。SpecPilot 项目可以作为参考：

- **如何稳定触发嵌套 Skill** — 不是让 Agent 依靠文档描述做了“看起来像触发了 Skill”的操作（比如根据 Skill 描述写了文件），而是真正触发
  Skill（核心特征：CC 上有 Skill 触发的打印）。SpecPilot 中会触发大量来自 OpenSpec 和 Superpowers 的能力，这段 Prompt 是怎么写的？

- **如何让组合 Skill 多阶段自动流转** — 不是靠人工介入。SpecPilot 的 5 阶段流程，除必要的用户选择项外，核心流程能够自动进行
  Skill 触发，同时状态机机制也能保障状态扭转的可靠性。

- **如何把 Spec 生命周期做成可恢复流程** — SpecPilot 会把 OpenSpec 的 change/spec 制品与 Superpowers 的设计、计划文档关联起来，并通过
  `.comet.yaml` 记录阶段、执行模式、验证结果和归档状态，让 Agent 中断后能够继续，而不是重新翻文档猜进度。

- **如何把文档同步从“用户提醒”变成自动化** — SpecPilot 将 handoff、状态更新、校验和归档同步放进脚本化流程，减少“记得更新 design
  doc”“记得同步 spec”“记得归档 change”这类反复提示。

- **如何设计 Agent 可执行的守护条件** — SpecPilot 的阶段退出不是简单相信 Agent 说“完成了”，而是通过 `comet-guard.sh`、
  `comet-yaml-validate.sh`、`comet-state.sh` 等脚本检查任务、状态字段、验证证据和归档条件，满足条件后才允许推进。

- **如何做跨平台 Skill 分发和安装** — SpecPilot 支持多种 AI 编码平台、项目级/全局安装、中文/英文 Skill 选择，以及平台差异化目录（例如
  Antigravity 的项目级和全局路径不同），可以作为 CLI 安装器和 Skill 打包结构的参考。

- **如何把 shell 脚本写成 Agent 工作流基础设施** — SpecPilot 的脚本需要兼容 macOS、Linux、Windows Git Bash，处理 hash、YAML
...
<details>
<summary><code>specpilot init [path]</code> — 初始化 SpecPilot 工作流</summary>

为选定的 AI 编码平台初始化 OpenSpec、Superpowers 和 SpecPilot 技能。
...
<details>
<summary><code>specpilot status [path]</code> — 显示活跃更改和下一步命令</summary>

显示活跃更改、任务进度，以及推荐的下一步 SpecPilot 工作流命令。
...
<details>
<summary><code>specpilot doctor [path]</code> — 诊断 SpecPilot 安装健康状态</summary>

检查项目级/全局安装、工作目录、已安装技能、脚本和 SpecPilot 状态文件。
...
<details>
<summary><code>specpilot update [path]</code> — 更新 SpecPilot 包和技能</summary>

更新 npm 包，并刷新已检测到的项目级/全局 SpecPilot 技能。
...
<details>
<summary><code>specpilot uninstall [path]</code> — 卸载 SpecPilot 技能、规则和钩子</summary>

安全移除 SpecPilot 分发的技能、规则和钩子，保留用户自定义的钩子和非 SpecPilot 配置。
...
```bash
specpilot uninstall              # 交互式 — 显示已安装目标，确认后卸载
specpilot uninstall --force      # 非交互式 — 直接移除所有内容
specpilot uninstall --scope project  # 仅移除项目级安装
```
...
|-------------------|------|
| `specpilot --help`    | 显示帮助 |
| `specpilot --version` | 显示版本 |

## 支持平台

`specpilot init` 支持 29 个 AI 编码平台：
...
`specpilot init` 完成后，三组技能将被安装到所选平台的 `skills/` 目录：

### SpecPilot 技能

SpecPilot 目前保留已安装工作流技能的 `/comet*` 前缀，用于内部工作流层、状态文件、脚本和各平台集成；npm 包为 `specpilot-ai`，CLI 命令为 `specpilot`。
...
SpecPilot 使用解耦状态架构，YAML 文件独立管理：

| 文件               | 归属       | 用途              |
|------------------|----------|-----------------|
| `.openspec.yaml` | OpenSpec | Spec 生命周期、变更元数据 |
| `.comet.yaml`    | SpecPilot    | 工作流阶段、执行模式、验证状态 |

所有状态和运行阶段都通过脚本更新，并且会在每个阶段退出前校验任务是否真实完成。相比于将复杂状态管理写在 Skill
文本中，脚本化状态机能更稳定地保障阶段流转、YAML 正确性和断点恢复；Agent 只需要通过 SpecPilot 内置命令读取状态，就能知道当前
Spec 处于哪个阶段。
...
SpecPilot 通过自动化状态转换确保 agent 执行可靠性：
...
├── .claude/skills/              # 平台技能目录（SpecPilot + OpenSpec + Superpowers）
│   ├── comet/SKILL.md
│   │   └── scripts/
...
SpecPilot 支持在 Design → Build 阶段交接时进行上下文压缩。启用后，`comet-handoff.sh` 会生成精简的上下文包，在不影响实现正确性的前提下，将
...
在 [SpecPilot 仓库](https://github.com/jianghuife/SpecPilot) 查看开发进展与即将推出的功能。
...
<a href="https://github.com/jianghuife/SpecPilot/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=jianghuife/SpecPilot&max=999&columns=12&anon=1" />
</a>
